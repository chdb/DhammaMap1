...
nonceLen = config.appCfg.NonceLEN()
token_span    = lengthVdr([nonceLen, nonceLen+5])
...
def captchaVdr(captchaStr, ipa):
    ...
    if not result['success']:
        logging.warning('recaptcha failed with: %r', result)
...
def simpleArithmeticExpr(expr):
    '''Sanitize arithmetic input.  If we allow simple arithmetic expressions, ie functions f() of an integer variable n,
    then we need to check that all values f(n) are within range for n in [0, max]'''
    # len(fnStr) <= max
    arithExpr_span.fn(expr)

    maxbad = 50 #TODO put in cfg
    try:
...
def eval_expr(expr, vals):
    ...
    def limited_power(a, b):
        ...
        if abs(b) > 20:  abort('excessive exponent: %r' %b)
        if abs(a) > 100: abort('excessive mantissa: %r' %a)
    ...
    def eval_(x): # x:= node in recursive tree
        if isinstance(x, ast.Num):    return x.n                                            # actual number
        if isinstance(x, ast.BinOp):  return ops[type(x.op)](eval_(x.left), eval_(x.right)) # <left> <operator> <right>
        if isinstance(x, ast.UnaryOp):return ops[type(x.op)](eval_(x.operand))              # <op> <operand> EG Node(-1) has {op: ast.USub,  operand:ast.Num(1)}
        if hasattr(x,'id'):
            if isinstance(x, ast.Name):
                if x.id in vals:      return vals[x.id]                                     #allow recognised names
