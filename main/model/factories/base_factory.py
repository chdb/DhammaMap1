...
class BaseFactory(NDBFactory):
    """ndbModelBase factory for factory_boy classes, for generating fixtures
    ...
    def create_batch(cls, size, **kwargs):
        """Firstly deletes all entries from datastore and then
         creates a batch of new entries"""
        ndb.delete_multi(cls._meta.model.query().fetch(keys_only=True))
        super(BaseFactory, cls).create_batch(size, **kwargs)

    @classmethod
    def _create(cls, model_class, *args, **kwargs):
        """Since we use factory.CREATE_STRATEGY this function will be called whenever creating instance of this class.
